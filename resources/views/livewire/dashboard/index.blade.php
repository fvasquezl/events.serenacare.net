...
<div wire:poll.60s="refresh">
    ...
    @else
        {{-- Botón de logout --}}
        <form method="POST" action="{{ route('logout') }}" class="fixed top-4 left-1/2 -translate-x-1/2 z-50">
            @csrf
            <button type="submit"
                    class="p-2 rounded-full text-white/0 hover:text-white/70 hover:bg-white/10 transition-all duration-300 cursor-pointer"
